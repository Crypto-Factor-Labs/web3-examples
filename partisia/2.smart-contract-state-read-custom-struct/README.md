...
We will make a query to select the active owner by deserializing the ownable state custom struct 
...
const activeOwner = await partisiaConnection.call(
    PBCChain.TESTNET, [1]
    ForkRegistryAbi, [2]
    "021948b37c0c7942106aac95c0129c059f44016cb3", [3]
    state => {
        const ownableState = state["ownable_state"].structValue(); [4]
        return ownableState.getFieldValue("owner").addressValue().value.toString("hex"); [5]
    }
);
```
...
for our particular need. The callback receives three arguments and for this example we are interested 
in the first one "state". This variable is a map containing the non-tree elements of
the smart contract state. If we check the image we showed earlier of the fork 
registry state we can see that we have a member called: "ownable_state"
in which we are interested, because it contains a field with the current owner address. 
The "state" map uses these names as keys and the values of the map
are a special type used to deserialize the state. To be able to deserialize a state value we
have to be aware of the type of the value. \
For example lets look into the owanble_state value which we are interested in. We access the value 
deserialization from the map using the key [4] and because we know the value is a custom struct
we can deserialize it to a temporary struct object that we can also query the fields of
using the "structValue" method of the deserialization object. \
To access the field "owner" we can then call the method "getFieldValue" using the field name for the 
struct which will return us another deserialization object which we can use to deserialize the field.
Knowing that the field is an address we can use the already shown method "addressValue()"
and encode the buffer as a "hex" string. [5] \
