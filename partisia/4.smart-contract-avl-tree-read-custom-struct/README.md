...
We will make a query to select the avl tree with tree id #0 from the smart contract 
...
const blockHeight = await partisiaConnection.call(
    ...
    (state, trees, namedTypes) => { [1]
        const blockTree = trees[0](HashTypeSpec, namedTypes["PbcMasterChainBlock"], true); [2]
        const targetBlock = blockTree.filter(x => x.key.hashValue().value.toString("hex") === targetBlockHash).pop(); [3]
        return targetBlock.value.structValue().getFieldValue("height")!.asBN().toString("hex"); [4]
    },
    [0] 
);
```
...
To query avl trees with custom struct values we have to provide some extra parameters on top of the ones used for
...
where the keys of the map are tree ids and for the custom struct we have to define the third argument "namedTypes"
which contains a map of struct defined in the smart contract as part of the state or rpc. The keys of namedTypes map
are struct names and the values are the type specs. [1] \
The map with trees only contains the trees with ids provided in the optional second parameter of the call function which
is an array of numbers ([0]).
We can find the treeId of an avl tree from the explorer, and for our example this is the number 0:
...
is a custom struct. For our example the key of the finalize partial chain epochs is a hash and the value is custom struct.
The type spec for the custom struct we get form the namedTypes parameter and the name of our struct is "PbcMasterChainBlock".
Because we are using custom type we are providing true as the final parameter for the builder. [2] \
...
To find the value in the tree for our chain, which in the example is a particular block hash,
we can run a normal filter over the array of key-value pairs with a check for the key deserialized as a hash and encoded in hex. [3] 
The type of that value is a struct defined in the smart contract called "PbcMasterChainBlock".
Structs on PBC are one of the special types, which we need to deserialize
using a predefined method, in this case "structValue" [4], which will return us the deserialiable struct with access to
